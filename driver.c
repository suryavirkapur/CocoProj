...
int main(int argc, char* argv[]) {
  ...
  printf("Enter 3 for parsing to verify the syntactic correctness of the input source code and printing the parse "
         "tree\n");
  printf("Enter 4 for printing the total time taken by your project code of lexer and parser to verify the syntactic "
         "correctness\n");
  ...
  while (1) {
    ...
    case 2: {
      ...
      initializeLexer(f);
      Token* t;
      while ((t = getToken()) != NULL) { // Remove the t->TOKEN_NAME != TK_ERR condition
        if (t->TOKEN_NAME == TK_COMMENT) {
          ...
          // printf("Line no. %d\t Lexeme %s\t Token %s\n", t->LINE_NO, t->LEXEME,
          // getTerminal(t->TOKEN_NAME)); printf("Line %d Error: Unknown pattern <%s>\n", t->LINE_NO,
          // t->LEXEME);
        } else {
